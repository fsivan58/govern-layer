...
int init_tmr1_int_frequency(float Fcyc, float scheduller_period)
{
    ...
    PR1_count = Fcyc * scheduller_period;
    ...
    else if((PR1_count/8) < 65535.0)
        {
            T1CONbits.TCKPS = 0b01;
            PR1 = (unsigned int)(PR1_count/8);
            return(0);
        }
        else if((PR1_count/64) < 65535.0)
            {
                T1CONbits.TCKPS = 0b10;
                PR1 = (unsigned int)(PR1_count/64);
                return(0);
            }
            else if((PR1_count/256) < 65535.0)
                {
...
unsigned int mem[3000];  // Word array for Task stacks
                ...
                unsigned int priority)
{
    ...
    task_stack_address = GiveMeMemory(mem_needed);
...
/* This function makes a TEST and SET operation on the file register 
   ...
   replays with a 0 in its second paameter (W1), otherwise it replays with 
   a 1.
