...
void printing_task()
{
    ...
    while(1)
    {
        /* There is no need to protect the execution of "printf" to gain 
         exclusive access to it, due to the priorities of the tasks. */
        printf("My identifier: %d\r\n", own_id);

        /* Task with higher priority will awake later, then, they will execute
         "wait" later too. */
        delay_until(clock() + 10 + own_id);
        wait(&sem);
    }        
}
...
void control_task()
{
    unsigned long period, wake_up_time;
    int x;
    unsigned long cont = 0;
       
    period = (unsigned long)(1.0 / tic_period());
    wake_up_time = clock() + period;
    
    delay_until(wake_up_time);
    
    while(1)
    {
        printf("-------------------: %lu\r\n", cont);
        
        cont += 1; /*There is not need to consider overflow */
...
int main(void) 
{
    ...
    for(x = 1; x <= 9; x++)
    {
       t_id = create_task(__builtin_tblpage(printing_task),
                          __builtin_tbloffset(printing_task), 150, x);
    }

  
    t_id = create_task(__builtin_tblpage(control_task),
                       __builtin_tbloffset(control_task), 150, 20);
